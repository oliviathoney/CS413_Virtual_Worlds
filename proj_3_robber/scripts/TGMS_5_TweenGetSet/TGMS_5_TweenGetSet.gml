...
TGMS_HIDE_WARNING TweenGroupGetTimeScale;
...
TGMS_HIDE_WARNING  TweenGroupSetTimeScale;
...
TGMS_HIDE_WARNING TweenGetDefault;
...
TGMS_HIDE_WARNING TweenSetDefault;
...
{	/// @func TweenGet(tween, "data_label")
	...
	*/
	...
	_dataLabel = TGMS_StringStrip(_dataLabel);
...
TGMS_HIDE_WARNING TweenSet;
function TweenSet(_t, _dataLabel) 
{	/// @func	TweenSet(tween[s], "dataLabel", value, ...)
	...
	*/
	...
	{
		_dataLabel = TGMS_StringStrip(argument[_pIndex++]);
		...
		if (_index == undefined)
		{
			if (is_string(_dataLabel))
			{
				if (string_length(_dataLabel) > 1 && global.TGMS.ShorthandTable[string_byte_at(_dataLabel, 1)])
				{
					...
					{
						_setValue = global.TGMS.ShortCodesEase[? TGMS_StringStrip(_dataLabel)];
					...
					{
						_setValue = global.TGMS.ShortCodesMode[? TGMS_StringStrip(_dataLabel)];
		...
		{   
		    switch(_index)
		    {
				...
				case TWEEN.EASE:
		    		...
		    		{
						_setValue =  global.TGMS.ShortCodesEase[? TGMS_StringStrip(_setValue)];
		    		}
