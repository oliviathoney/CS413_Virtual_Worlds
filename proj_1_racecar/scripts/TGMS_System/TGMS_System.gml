...
// This is used to hide Syntax Errors
#macro TGMS_HIDE_WARNING if(0) _ = 
...
function TGMS_Tween(script, args, tweenID)
{
	...
	// Make sure we have a o_SharedTweener object created -- this is inlined
	var _sharedTweener;
	if (instance_exists(o_SharedTweener))
	{
		_sharedTweener = o_SharedTweener.id;	
	}
	else
	{
		instance_activate_object(o_SharedTweener);
		...
		_sharedTweener = instance_create_depth(0,0,0,o_SharedTweener);
	}
	...
	else
	{
		...
		_t = TGMS.TweenDefault;				// Get default tween base
	...
	{		
		...
		//	}
		//}
		//else // IF Advanced Property
		
		//if (is_array(_tag)) // old -- now wrapped in struct
		if (is_struct(_tag))
		{
			_tag = _tag.data;
	...
	{	// Default group scale to 1.0 if it doesn't exit
		if (!ds_map_exists(TGMS.GroupScales, _t[TWEEN.GROUP]))
		{
	...
	//if (is_string(_t[TWEEN.EASE])) // IS THIS EVEN NEEDED??
	//{
	//	var _cache = TGMS.Cache[? _t[TWEEN.EASE]];
		
	//	if (is_undefined(_cache))
	//	{
	//		_cache = TGMS_StringStrip(_t[TWEEN.EASE], 0);	
	//	}
		
	//	_t[@ TWEEN.EASE] = TGMS.ShortCodesEase[? _cache];
	//}
	//else
	...
	// THIS SHOULD NOT BE NEEDED AS THE MODE IS ADJUSTED WHENEVER SET ELSEWHERE... but keeping this commented out here for now just in case I'm missing something
	//if (!is_numeric(_t[TWEEN.MODE]))
	//{
	//	//if (is_string(_t[TWEEN.MODE]))
	//	//{
	//	//	_t[@ TWEEN.MODE] = TGMS.ShortCodesMode[? TGMS_StringStrip(_t[TWEEN.MODE], 0)];
	//	//}
	//	//else
	//	if (is_array(_t[TWEEN.MODE]))
	//	{
	//		_t[@ TWEEN.CONTINUE_COUNT] = _t[TWEEN.MODE][1];
	//		_t[@ TWEEN.MODE] = is_real(_t[TWEEN.MODE][0]) ? _t[TWEEN.MODE][0] : TGMS.ShortCodesMode[? TGMS_StringStrip(_t[TWEEN.MODE][0], 0)];
	//	}
	//}
...
{	/// @func TGMS_TweenPreprocess(rawTween)
	...
	//var _dataSize = array_length(_pData);
	...
	repeat(_propCount)
	{
		...
		else // DYNAMIC PROPERTY
		{
			...
			if (_dotPos) //
			{
				...
				{
					_extData[  _extIndex] = TGMS_Variable_Struct_String_Set;
					_extData[1+_extIndex] = _pData[i-1]; // start
					_extData[2+_extIndex] = _pData[i] - _pData[i-1]; // change
