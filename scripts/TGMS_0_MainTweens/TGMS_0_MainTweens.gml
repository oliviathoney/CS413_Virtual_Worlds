...
{	/// @desc Tween a property between start/destination values (auto-destroyed)
	...
	*/
	...
	//if (is_string(_arg0) || is_array(_arg0)) // TODO: WHY IS THERE AN ARRAY CHECK??? Update: This was possibly to check for TP*() functions which return an array -- looking to have them return structs instead
	if (is_string(_arg0))
	{		
		var _args = array_create(argument_count);
		_args[0] = _arg0; // We already know the first argument...
...
{	/// @desc	Plays a tween previously created with TweenCreate()
	...
	*/
	...
	else
	{
		...
		if ((is_string(argument[1]) && global.TGMS.ShorthandTable[string_byte_at(argument[1], 1)]))
		{
...
TGMS_HIDE_WARNING  TweenMoreScript;
